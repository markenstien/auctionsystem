<?php 
	...
	if(filter_var($email,FILTER_VALIDATE_EMAIL)) {
		...
		$sql = mysqli_query($conn,"SELECT email FROM bidder_tbl WHERE email = '{$email}'");
				...
				if (isset($_FILES['image'])) {          //if file is uploaded 
					...
					if(in_array($img_extension,$extensions) === true) {
						...
						if(move_uploaded_file($tmp_name,"../Images/" . $newimagename)) {
							...
							$sql2 = mysqli_query($conn,"INSERT INTO bidder_tbl (unique_id, fname, lname, email, phone, password, image,otp,verification_status, Role)
							VALUES ({$random_id},'{$fname}','{$lname}','{$email}','{$phone}','{$password}','{$newimagename}','{$otp}','{$verification_status}','{$Role}')");
							if($sql2) {
								$sql3 = mysqli_query($conn,"SELECT * FROM bidder_tbl WHERE email = '{$email}'");
